...
import { deleteInterview, updateInterview } from '../../../../../API/interview';
...
const Singleinterview: FC<any> = ({ interviewData, id }) => {
  ...
  // delete interview
  const { mutate: deleteCInterview, isLoading } = useMutation(deleteInterview, {
    onSuccess: () => {
      queryClient.invalidateQueries(`${id} interviews`);
      toast({
        position: 'top-right',
        isClosable: true,
        render: () => (
  ...
  // update interview
  const { mutate: updateCInterview, isLoading: isUpdateLoading } = useMutation(
    updateInterview,
    {
      onSuccess: () => {
        queryClient.invalidateQueries(`${id} interviews`);
